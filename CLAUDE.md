...
- **Голосовые предупреждения** — `push_voice_alert(cam_id,text)`/`pop_voice_alert()` — потокобезопасная очередь (deque max 50) с кулдауном `VOICE_ALERT_COOLDOWN`(15с) на камеру. Тексты — чистые функции в `backend/tts/alert.py` (реэкспорт в `main.py`):
  ...
  - **Пропуск** (`state.approve`/`is_approved`, TTL `APPROVAL_DURATION`=300с) выдаётся по жесту ОК при полном СИЗ; пока активен — тревога на человека не срабатывает. Ручной сброс: `state.revoke_approval(gid)`, `POST /api/reid/persons/<gid>/revoke`, кнопка в `GalleryModal` (поле `pass_seconds_left` в `/api/reid/persons`).
