...
- **Голосовые предупреждения** — `push_voice_alert(cam_id,text)`/`pop_voice_alert()` — потокобезопасная очередь (deque max 50) с кулдауном `VOICE_ALERT_COOLDOWN`(15с) на камеру. Текст формирует `_build_voice_text` в `main.py` (имя нарушителя по Re-ID + перечень отсутствующих СИЗ), пушится из `detection_loop` при старте новой записи нарушения. Фронт поллит `/api/voice_alert` (`useVoiceAlerts`) и озвучивает через Web Speech API. ⚠️ Синтез — **в браузере оператора средствами ОС**; без установленного русского TTS-голоса озвучка пропускается (см. раздел 4), бэкенд тут ни при чём.
...
- **Голосовые предупреждения требуют русского TTS-голоса в ОС оператора.** Синтез делает браузер (Web Speech API), не бэкенд. `getVoices()` в Chrome пуст на первом вызове (голоса грузятся асинхронно) — `useVoiceAlerts` поэтому кэширует их по событию `voiceschanged`. Если русского голоса в системе нет, кириллицу читает англ. голос «быстро и неразборчиво» → озвучка намеренно пропускается с предупреждением в консоли (`[voice] Русский голос синтеза не найден`). Лечится установкой голоса в ОС (Windows: речевой пакет Microsoft Irina; Linux: RHVoice).
