...
- **Голосовые предупреждения** — `push_voice_alert(cam_id,text)`/`pop_voice_alert()` — потокобезопасная очередь (deque max 50) с кулдауном `VOICE_ALERT_COOLDOWN`(15с) на камеру. Тексты — чистые функции в `backend/tts/alert.py` (реэкспорт в `main.py`):
  ...
  - Фронт поллит `/api/voice_alert` (`useVoiceAlerts`). **Синтез — на бэкенде через Piper** (`backend/tts/`); фронт получает готовый WAV с `/api/voice_alert_audio?text=...` и играет через `<audio>`. **Fallback:** 503 (нет piper/модели/`TTS_ENABLED=false`) → Web Speech API браузера (зависит от русского TTS-голоса ОС, см. раздел 4).
