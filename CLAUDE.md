...
- **План — моделью Opus, исполнение — моделями послабее.** Для сложных/многошаговых задач сначала Opus строит план (анализ, декомпозиция, архитектурные решения) и оформляет его **md-чеклистом в репо**; затем исполнение по пунктам можно отдать модели послабее (Sonnet/Haiku — через subagent с `model: sonnet`/`haiku` или переключением модели). Исполнитель идёт по чеклисту и отмечает прогресс по ходу. Opus сильнее в рассуждении, но дороже; механическое исполнение готового плана топ-модели не требует.
...
```
- `docker-compose.override.yml` (dev, применяется автоматически) bind-маунтит Python-код и **прогоняет pytest перед стартом** (`pytest tests/ && python app.py`), плюс ставит FLASK_DEBUG=1. `frontend/dist` НЕ бинд-маунтится — фронтенд собирается целиком внутри образа (Stage 1 Dockerfile'ов, `npm ci && npm run build`), поэтому `docker compose ... up --build` собирает всё (бэк + фронт) одной командой без npm на хосте.
- После правок Python достаточно `restart` (код примонтирован). После правок React (или файлов фронтенда) нужен пересобранный образ — `docker compose --profile cpu up --build -d` (npm-сборка идёт внутри Docker).
...
- `run_detection(frame, model)` → `model.track(..., persist=True, tracker=bytetrack_custom.yaml)`, возвращает dict: `persons`, `person_track_ids`, `helmets`, `masks`, `vests`, `cones`. Классы фильтруются по русским именам из `CLASS_NAMES`.
  - **Двойной порог уверенности**: детекция гоняется по `min(CONF_THRESH, PPE_CONF_THRESH)`, затем фильтр покласово — люди/конусы по `CONF_THRESH`(0.75), предметы СИЗ (каска/маска/жилет) по более мягкому `PPE_CONF_THRESH`(0.5, env). Иначе при едином 0.75 каска (объект мельче человека) часто отбрасывалась до сопоставления → ложное «нет каски» у человека В каске → ложный голос/нарушение. Ниже PPE-порог → больше ловится каска, но растёт риск ложного «СИЗ есть»; клампится сверху `CONF_THRESH`.
...
- **Пер-зонные требования СИЗ** (`zones.required_ppe`): какие СИЗ обязательны для человека = объединение `require_ppe` danger/restricted-зон, в которых он стоит (пусто → СИЗ не нужны); вне зон и в зоне по конусам — глобальный дефолт `PPE_REQUIRED_DEFAULT` (`config.py`, env). Это управляет `missing`/`fully_equipped`/нарушением и компактным статусом (необязательный СИЗ не считается отсутствующим). Демо/выставка: `PPE_REQUIRED_DEFAULT=mask` (только маска) или `PPE_REQUIRED_DEFAULT=` (СИЗ не нужны нигде), либо нарисовать зону с нужным `require_ppe`.
  - **Рантайм-настройка дефолта из UI** (Настройки → «Обязательные СИЗ для пропуска», чекбоксы каска/маска/жилет): `GET|PUT /api/ppe-required` правит `PPE_REQUIRED_DEFAULT` **на месте** (`[:]=`, чтобы импортированная в `main.py` ссылка видела изменения без рестарта) и персистит в `data/ppe_required.json` (переопределяет env-дефолт при старте). Так задаётся, какой комплект СИЗ нужен для выдачи пропуска по жесту «ОК» (`fully_equipped`). Пустой набор → пропуск без СИЗ. Фронт: панель «Статус проверки СИЗ» (`ControlPanel`) показывает строки только для требуемых средств (`ppeRequired` в `CameraContext`).
...
- **Голосовые предупреждения** — `push_voice_alert(cam_id,text)` (потокобезопасный кольцевой буфер deque max 50, кулдаун `VOICE_ALERT_COOLDOWN`(15с) на камеру) + **курсорное** чтение `get_voice_alerts_since(after_seq)`: каждому алерту присваивается монотонный `seq`, клиент читает «всё после своего курсора» и НЕ извлекает из общей очереди (раньше `pop_voice_alert` «съедал» алерт у других вкладок, а несколько камер сериализовались по одному на опрос). Тексты — чистые функции в `backend/tts/alert.py` (реэкспорт в `main.py`):
  - `build_voice_text(voice_violators)` — тревога. `voice_violators` собирается ПОПЕРСОННО в `process_frame`: `(имя, его_отсутствующие_СИЗ)` только для людей в опасной зоне без пропуска и без требуемых СИЗ. Имя и список missing — одного человека (раньше СИЗ агрегировались по всем в кадре → человек В каске «слышал», что у него нет каски). Пушится на каждом кадре зонного нарушения **независимо от записи клипа** (троттлинг кулдауном).
  - `build_approved_voice_text(names)` — спокойная отметка «{имя} в опасной зоне, в средствах защиты. Причин для внимания нет» для людей с активным **пропуском** в зоне. Озвучивается ТОЛЬКО при входе нового (дедуп по `_voice_approved_seen[cam_id]` — set global_id), иначе повторялась бы каждый кулдаун. Нарушение имеет приоритет (calm пушится только если нет нарушителей).
  - **Пропуск** (`state.approve`/`is_approved`, TTL `APPROVAL_DURATION`=300с=5мин) выдаётся по жесту ОК при полном СИЗ; пока активен — тревога на человека не срабатывает. **Ручное управление из `GalleryModal`** (поле `pass_seconds_left` в `/api/reid/persons`): выдать — `state.grant_approval(gid)` / `POST /api/reid/persons/<gid>/approve` (кнопка 🎫, видна всегда); сбросить — `state.revoke_approval(gid)` / `POST .../<gid>/revoke` (кнопка 🎫✕, когда пропуск активен).
  - Фронт поллит `GET /api/voice_alert?after=<seq>` (`useVoiceAlerts`) → `{alerts, cursor}`: получает все алерты новее курсора и проигрывает их **последовательно** (очередь + флаг, без наложения речи нескольких камер); несколько вкладок/камер обслуживаются независимо. Первый опрос (без `after`) лишь инициализирует курсор, без переигрывания бэклога. **Синтез — на бэкенде через Piper** (`backend/tts/`): фронт получает готовый WAV с `/api/voice_alert_audio?text=...` и играет через `<audio>`. **Fallback:** 503 (нет piper/модели/`TTS_ENABLED=false`) → Web Speech API браузера (зависит от русского TTS-голоса ОС, см. раздел 4).
...
- `FaceRecognizer` (`recognizer.py`) — InsightFace `buffalo_l`, провайдеры CUDA→CPU, 3 попытки загрузки (чистит пустую папку модели и докачивает). **Офлайн-старт:** если модели нет локально и сети нет (`netutil.is_online()`) — сразу `RuntimeError` (Re-ID отключается в `main.py`), без зависания на download-ретраях; ретраи со сном — только онлайн. `detect_faces` возвращает `(bbox, нормализованный эмбеддинг 512d, quality)`. `quality` = `det_score*0.7 + size_score*0.3`, клампится в [0.35, 1.0].
...
- `FaceGallery` (`gallery.py`) — pickle-хранилище `data/face_gallery.pkl`. Ключевое:
  ...
  - **Защита якоря** `_append_embedding`: при переполнении (`max_embeddings`=`REID_MAX_EMBEDDINGS`=30) выбрасывается **самый старый по времени** НЕ-якорный эмбеддинг (по `embedding_ts`), а не вслепую индекс 1; первый (эталонный) эмбеддинг неприкосновенен. «Запоминание со всех сторон»: до 30 ракурсов на личность.
  ...
  - **Гейт сохранения по якорю** (`REID_STORE_MIN_SIM`, env, дефолт 0.45): **развязка recall и storage** — матчинг/липкость (recall) остаётся мягким (REID_STICKY_MIN=0.28, чтобы не вернуть баг «всё слетает на Гость_N»), но сохранение эмбеддинга строгое. `_append_embedding` дописывает эмбеддинг к личности, только если `cos(emb, anchor) ≥ REID_STORE_MIN_SIM`, где `anchor = embeddings[0]` — эталонный неизменный первый эмбеддинг. Защита от ByteTrack ID-switch: при пересечении/окклюзии трек «человека A» может временно оказаться под «человеком B»; для ArcFace (buffalo_l) косинус между разными людьми обычно 0.28–0.45 — гейт 0.45 останавливает дописывание чужого лица в галерею другой личности. Гейт по якорю (а не по max ко всем) устойчив к дрейфу: единичный просочившийся чужой эмбеддинг не становится новым «аттрактором». Применяется только при дописывании (`_append_embedding`), НЕ при создании якоря в `match_or_register` и НЕ в `merge_entries`. 0/отрицательное — выключено (старое поведение).
  - **Затухание памяти по времени** (`embedding_ts` + `prune_old_embeddings`): у каждого эмбеддинга есть таймстамп добавления; лишние ракурсы старше `REID_EMB_MAX_AGE_DAYS`(env, дефолт 30 дн.) **постепенно стираются** (TTL на эмбеддинг), но **якорь сохраняется всегда** — образ человека остаётся в памяти, выцветают только детали. Запускается при старте (`main.py`) и периодически в `_heartbeat_loop` раз в `REID_EMB_CLEAN_INTERVAL`(env, 3600с); запись на диск — только при реальном удалении. `0`/отрицательное — затухание выключено. Старые pickle без `embedding_ts` дозаполняются `last_seen` при загрузке (`_embedding_ts`).
  - `match_or_register` / `add_observation` / `merge_entries` (auto-merge дубликатов) / `cleanup_old`: при `REID_MAX_AGE_DAYS`=0 авто-удаление **целых личностей** отключено («навсегда»); затухание отдельных эмбеддингов по времени — отдельный механизм (`REID_EMB_MAX_AGE_DAYS`, см. выше).
...
- **Body Re-ID — опознание «со спины»** (`backend/reid/body.py::BodyRecognizer` + поля `body_embeddings` в галерее + ветки в `get_global_id`). Дескриптор внешнего вида (одежда+силуэт) — одинаков с лица и со спины, L2-норма, косинус. **Два бэкенда (выбор авто в `__init__`):**
  - **DEEP (основной)** — глубокая Re-ID сеть **OSNet** (`osnet_x0_25_msmt17`, ONNX, onnxruntime — тот же, что для InsightFace; CUDA→CPU). 512-мерный семантический дескриптор, устойчивый к ракурсу. Вход 128×256, RGB, ImageNet-норма. **Модель экспортирована со статическим батчем 16** → кропы людей гоняются пачками по 16: `extract_batch()` даёт **один инференс на кадр**, не на человека (`process_frame` зовёт его один раз до цикла людей). Модель (~0.9 МБ) в `models/osnet/` (`.gitignore`), идемпотентная докачка `backend/reid/body_models.py::ensure_body_model` из HuggingFace (`REID_BODY_MODEL_URL`); `REID_BODY_OFFLINE=1` запрещает сеть (тесты).
  - **COLOR (fallback)** — цветовой дескриптор (HSV-гистограммы торса/ног, 256-d), без внешних зависимостей. Включается **мягкой деградацией**, если OSNet/onnxruntime недоступны (как Piper TTS → Web Speech). Дескрипторы разной размерности (deep 512 vs color 256) **не смешиваются** — галерея (`_body_sims`) сравнивает только эмбеддинги одинаковой длины.
  - **Пороги под бэкенд:** `BodyRecognizer.{backend,match_threshold,diversity_max_sim}` → `state.configure_body()` (вызывается из `main.py` после инициализации). Deep: `REID_BODY_MATCH_THRESHOLD`=0.60 / diversity 0.90; color: `REID_BODY_MATCH_THRESHOLD_COLOR`=0.82 / 0.97. Порог матча подбирать вживую.
  - **Логика (одинакова для обоих бэкендов):** пока лицо видно, тело личности **запоминается** (`_store_body`, троттлинг `REID_BODY_STORE_INTERVAL`, diversity-гейт); когда лица нет и трек новый — личность **восстанавливается** через `FaceGallery.match_body`. Консервативно: только восстанавливает (не перекрывает лицо) и не присваивает личность, активную на другом треке (`_gid_active_on_other_track`). Дескриптор берётся с **чистого кадра** (`clean_frame` в `process_frame`, до отрисовки — иначе оверлеи исказят дескриптор). Управляется тем же тумблером Re-ID («faces»). `BodyRecognizer.extract()` изолирован — deep-бэкенд заменяем на другую Re-ID модель (OSNet x1_0/torchreid), сохранив интерфейс (L2-нормированный вектор фиксированной длины или None).
  - **Персистентность и TTL тела:** дескрипторы тела имеют таймстампы (`body_ts`, параллельно `body_embeddings`) и **сохраняются на диск**, поэтому узнавание «со спины» переживает перезапуск. Запись не на каждый кадр: `add_body`/«липкое» дописывание лиц ставят **грязный флаг** (`_dirty`), а `FaceGallery.flush()` сбрасывает pickle только при реальных изменениях — периодически в `_heartbeat_loop` (раз в `REID_FLUSH_INTERVAL`(env, 30с)) и **гарантированно при `stop_live`**. TTL у тела **короткий** (`REID_BODY_MAX_AGE_DAYS`, env, дефолт 2 дня) и **без защиты якоря** (в отличие от лиц): внешний вид завязан на одежду и устаревает быстро — `prune_old_bodies` (старт + heartbeat-таймер) стирает все дескрипторы старше TTL; устарели все → личность не опознаётся со спины, пока её снова не увидят в лицо. Старые pickle без `body_ts` дозаполняются `last_seen` при загрузке.
...
### 2.6.1. Отслеживание перемещений (`backend/tracking/movement.py`)
Кто сидит на месте (и сколько), кто встал из-за стола и куда пошёл. **Без новой ML-модели** — чистая геометрия поверх готовых детекций людей: тестируется без кадров/GPU (как `MotionDetector`/`DetectionState`).
- **`MovementTracker`** (по экземпляру на камеру, `_movement_trackers` в `main.py`, лениво через `_get_movement_tracker`, сброс в `start_live`, снятие в `remove_camera`/`rename_camera`). `update(persons, now)` принимает список `{key, box, name}` всех видимых за кадр людей и возвращает `MovementInfo` в том же порядке (`state` sitting/moving, `seated_seconds`, `anchor`, `trail`, `just_stood_up`/`just_sat_down`); попутно чистит ушедших по TTL (`PERSON_EXPIRY_SEC`=60с). Пороги читаются **живыми** через `get_detection_setting` (hot-reload из UI), явные значения в конструкторе их переопределяют (тесты, `tests/test_movement.py`).
- **Ключ траектории** (`mkey` в `process_frame`): `g{global_id}` если `global_id>0` (переживает переинициализацию трека и узнавание «со спины»), иначе `t{track_id}` (режим «только люди», лица выключены). **Миграция идентичности** (`_migrate_identity`, по `track_id`): когда лицо распозналось, ключ трека меняется `t{track_id}→g{global_id}` — трекер **переносит** накопленное состояние (место, время «сидит», след) на новый ключ, а не сбрасывает таймер. Мигрирует ТОЛЬКО в глобальный ключ (`g…`), защита от переиспользования ByteTrack `track_id` под новым человеком (переход в `t…` не крадёт чужое место). Кросс-окклюзионная непрерывность (новый `track_id`, тот же `global_id`) работает автоматически — ключ `g{gid}` общий.
- **Мультикамерность:** пространственные треки — **по камере** (`_movement_trackers[cam_id]`), т.к. координаты локальны для камеры, а общий трекер по `global_id` при перекрывающихся FOV давал бы трэшинг (потоки камер дрались бы за один трек). Идентичность — **глобальная**: снапшот несёт `global_id` (и имя из галереи), поэтому одного человека можно **сшить между камерами** на фронте. `/api/movement` группирует по `cam_id`.
- **Конечный автомат** (точка ног = низ-центр bbox; пороги масштабируются **ростом bbox** → не зависят от разрешения/дистанции): `sitting` — точка ног держится у «якоря места» (anchor) в пределах `movement_still_frac`·рост, копится непрерывное `seated_seconds`; `moving` — сместился от якоря дальше `movement_move_frac`·рост (`just_stood_up`), рисуется траектория ухода; замер на новом месте дольше `movement_settle_sec` → новый якорь (пересел). Микродрейф места (катящийся стул) — мягкий EMA-подтяг якоря. След обрезается по `movement_trail_sec` (мин. 2 точки).
- **Отрисовка** (`renderer.py`: `draw_trajectory`/`draw_seat_marker`/`draw_movement_badge`) идёт в `process_frame` отдельным проходом после цикла людей (трекеру нужен весь кадр): полилиния следа (зелёная — сидит, жёлтая — идёт, cv2 без PIL), крест-якорь места, бейдж «Сидит MM:SS» / «Идёт». Снапшот (имя/статус/время) кладётся в `state.set_movement(cam_id, ...)` для `/api/movement`.
- **Гейт**: `DETECT_MODES["people"] and DETECT_MODES["movement"]` (см. 2.11). Работает и в режиме «только люди».
...
- **`api/detection.py`** (`configure_detection_routes`): `POST /start`, `POST /stop`, `/video_frame/<cam>`, `/video_feed[/<cam>]`, `GET|PUT /api/detect-modes`, `GET|PUT /api/ppe-required` (обязательные СИЗ для пропуска, см. 2.3), `GET /api/status`, `GET /api/voice_alert`, `GET /api/notifications` (очередь UI-уведомлений), `GET /api/movement` (снапшот перемещений: кто сидит/сколько, кто встал; `?cam_id=<id>` — по камере, см. 2.6.1), `/detection_log`, `/export_logs` (CSV с BOM), `POST /upload` (детекция на загруженном фото/видео).
...
`DETECT_MODES = {people, ppe, faces, movement}` (`backend/config.py`), персистятся в `data/detect_modes.json`, меняются через `PUT /api/detect-modes` / UI. Если все выключены — YOLO пропускается целиком, но **стрим не останавливается**: `process_frame` отдаёт сырой кадр без оверлеев (ранний `return` возвращает полный кортеж из 7 элементов — иначе воркер падал на распаковке и переставал писать в `annotated_buffers` → `/video_frame` отдавал 204 → фронт «NO SIGNAL»). Выключение `people` каскадно гасит `ppe`, `faces` и `movement`. Переключение `faces` стартует/глушит face-воркеры на лету. `movement` (отслеживание перемещений, см. 2.6.1) строится поверх детекции людей. **Режим «только люди»** (ppe+faces выкл.): люди рисуются нейтральной рамкой без статусов/пропуска — детекция жеста не запускается (иначе при пустом списке требуемых СИЗ жест «ОК» выдавал «пропуск» всем); траектории/время на месте при этом продолжают работать, если включён `movement`.
...
- **Перемещения** (см. 2.6.1): режим `movement` — тумблер «Перемещения» в `Header` (дропдаун «Детекция», + каскад people-off). Сама траектория/бейджи рисуются бэкендом прямо в кадр (фронт не нужен для их показа). `DispatcherPanel` дополнительно показывает секцию «ПЕРЕМЕЩЕНИЯ» (имя, статус сидит/идёт, время на месте) — собственный лёгкий поллинг `api.getMovement(cam)` при открытой панели и `isRunning` (снапшот эфемерный, в контекст не тянется).
- `src/api/client.ts` — HTTP-клиент с авто-refresh JWT. `src/contexts/` — Auth, Camera. `src/hooks/` — useBreakpoint, useOrientation, useClock, useCameras, useLogs, **useVoiceAlerts** (поллит `/api/voice_alert?after=<seq>` по курсору, проигрывает алерты последовательно: основной путь — WAV с бэкенда Piper, фоллбэк — Web Speech API; голоса Web Speech кэшируются по `voiceschanged` — иначе `getVoices()` пуст на первом вызове и кириллицу читает англ. голос), **useServerNotifications** (поллит `/api/notifications`, шлёт в верхнюю панель `Notifications`).
...
- **Автообнаружение камер** (`backend/discovery.py`) — поиск RTSP-камер в локальной сети: ONVIF WS-Discovery (multicast UDP 3702, raw-сокет без зависимостей; Probe шлётся дважды и ответы собираются весь таймаут — одиночный UDP терялся, камеры находились «через раз») + скан подсети **двухфазный**: (1) широкий TCP-скан порта 554 (пул 128, таймаут 1с + 1 повтор — WiFi-пики латентности), (2) классификация только откликнувшихся хостов. Пути проверяются **лёгким RTSP DESCRIBE** (`rtsp_describe_status`, мс на путь, НЕ занимает RTSP-сессию камеры — у камер лимит одновременных подключений, тяжёлая проба при живом стриме отклонялась): `_probe_host` классифицирует хост: **открытая** (какой-то путь ответил 200 без логина — `requires_auth=False`, есть `rtsp_url`) / **запароленная** (401/407, либо порт 554/ONVIF без открытого пути — `requires_auth=True`, `rtsp_url=None`) / не камера. Полное открытие потока `probe_rtsp` (OpenCV; ⚠️ таймауты передаются В КОНСТРУКТОР `VideoCapture` — `cap.set(...TIMEOUT...)` после создания не действует, конструктор блокируется на дефолтном ffmpeg-таймауте в десятки секунд) — только фоллбэк для молчащих на DESCRIBE хостов и финальная проверка кредов в `probe_rtsp_auth` (пути там предварительно сортируются `prioritize_paths` по DESCRIBE-статусам: 200/401 — первыми, 404 — в конец). `discover_cameras(add)`/`autodiscover_and_add` в `main.py`; API `POST /api/cameras/discover` (возвращает и открытые `status=new/added`, и запароленные `status=locked`; `add=True` автодобавляет только открытые; уже добавленные — `exists` по IP). **Запароленные:** UI показывает 🔒 и форму логин/пароль → `POST /api/cameras/discover/auth {ip,username,password,port?,name?}` → `add_authenticated_camera` подбирает рабочий путь (`probe_rtsp_auth`/`auth_candidate_urls` — креды URL-кодируются) и добавляет камеру (URL с кредами в `cameras.json`, как у любой RTSP-камеры). Кнопка «Найти камеры» в `CameraManager` (модалка + страница настроек). Автозапуск при старте — флаг `CAMERA_AUTODISCOVER` (фон, по умолчанию off). ⚠️ **Скан подсети в Docker bridge:** `local_ip()` внутри контейнера возвращает docker-адрес (172.x), а не LAN с камерами, поэтому подсеть для скана задаётся явно (env `DISCOVERY_SUBNET`, список через запятую: `192.168.0`, `a.b.c.0/24`) и/или автоматически выводится из IP уже добавленных камер (`scan_prefixes`/`ips_from_sources` в `discover_cameras`). Сам скан из bridge работает — исходящий TCP на LAN проходит через docker SNAT; host-сеть нужна только для **ONVIF-multicast** (`network_mode: host`, несовместим с `ports`). Чистые помощники (`ips_from_ws_responses`/`candidate_urls`/`subnet_hosts`/`prefix_of`/`ips_from_sources`/`normalize_subnets`/`hosts_for_prefixes`/`parse_rtsp_status`/`prioritize_paths`) тестируются без сети. **Офлайн-устойчивость LAN-определения** (`backend/netutil.py::lan_ipv4s`): `local_ip()`/`scan_prefixes` берут приватные IPv4 со ВСЕХ интерфейсов хоста (psutil, фоллбэк `getaddrinfo`), а не только connect-трюк к `8.8.8.8` — он без маршрута по умолчанию (LAN без WAN) даёт None. Так скан подсети работает и без интернета, и при нескольких LAN-интерфейсах.
...
- Рантайм-состояние в `data/`: `cameras.json`, `cameras_config.json`, `detect_modes.json`, `ppe_required.json` (обязательные СИЗ для пропуска, см. 2.3), `detection_settings.json` (рантайм-настройки детекции, см. ниже), `face_gallery.pkl`, `ppe.db`, `jwt_secret.txt`.
- **Рантайм-настройки детекции** (`config.py`: `DETECTION_SETTINGS` + `DETECTION_SETTINGS_SPEC`, API `GET|PUT /api/detection-settings`, UI «Настройки → Детекция и логика», компонент `DetectionSettings.tsx`). Набор параметров детекции/логики, правится из UI **на лету без рестарта**: `person_conf`/`ppe_conf` (пороги уверенности), `min_cones`/`zone_expand_px` (зона по конусам), `ppe_top_ratio` (засчёт СИЗ по высоте), `approval_duration`, `gesture_cooldown`, `voice_alert_cooldown`, `movement_still_frac`/`movement_move_frac`/`movement_settle_sec`/`movement_trail_sec` (отслеживание перемещений, см. 2.6.1). **Механизм:** точки использования читают значения ЖИВЫМИ через `get_detection_setting(key)` на каждом кадре/вызове (`engine.run_detection`/`parse_detection_results`/`get_danger_zone`/`has_item_on_person`, `batch_worker._base_conf`, `state.approve`/`can_gesture`/`push_voice_alert`, `tracking.movement.MovementTracker`, `/upload`), поэтому изменения применяются мгновенно (per-camera YOLO передаёт `conf` на каждом `model.track`). Константы (`CONF_THRESH` и т.д.) остаются ДЕФОЛТАМИ и обратной совместимостью импортов/тестов. `update_detection_settings(patch)` валидирует/клампит по спеке (`_coerce_clamp_setting`: тип + [min,max]) и мутирует словарь **на месте** (как `PPE_REQUIRED_DEFAULT[:]`), персист в `data/detection_settings.json`. **Спека — единый источник** для бэк-валидации И для генеративной отрисовки UI (label/desc/min/max/step/unit/group): новый тумблер = одна запись в спеке + чтение через `get_detection_setting` в точке использования, фронт не трогаем.
- Env: `JWT_SECRET`, `ADMIN_USERNAME`/`ADMIN_PASSWORD`/`ADMIN_EMAIL`, `INSIGHTFACE_ROOT`, `VITE_API_TARGET` (фронт), `BACKEND_URL` (nginx-фронт). Frigate-слой (2.13): `MOTION_DETECTION_ENABLED`, `MQTT_ENABLED`/`MQTT_HOST`/`MQTT_PORT`/`MQTT_USER`/`MQTT_PASSWORD`/`MQTT_TOPIC_PREFIX`/`MQTT_HA_DISCOVERY`. NVR (2.7.1): `RECORD_ENABLED`, `RECORD_MODE`(motion/continuous), `RECORD_DIR`, `RECORD_SEGMENT_SEC`, `RECORD_RETAIN_DAYS`, `RECORD_MAX_DISK_PERCENT`, `RECORD_MOTION_GRACE_SEC`, `RECORD_CLEAN_INTERVAL_SEC`. Зоны: `PPE_REQUIRED_DEFAULT` (список через запятую: helmet,mask,vest; дефолт обязательных СИЗ вне зон). Автообнаружение: `CAMERA_AUTODISCOVER`, `DISCOVERY_USE_ONVIF`, `DISCOVERY_USE_SCAN`, `DISCOVERY_ONVIF_TIMEOUT`, `DISCOVERY_SUBNET` (подсети для скана через запятую — обязательно в Docker bridge, см. 2.13). Re-ID: `REID_STORE_MIN_SIM` (дефолт 0.45 — строгий гейт сохранения эмбеддинга по якорю, защита от ByteTrack ID-switch; 0/отрицательное — выключен, см. §2.5).
...
- **Фронтенд собирается ТОЛЬКО внутри Docker-образа**, `frontend/dist` намеренно не бинд-маунтится в `docker-compose.override.yml` (раньше маунтился и при несобранном на хосте `frontend/dist` — он в `.gitignore` — подменял собранный в образе фронтенд пустой папкой; молча отдавался нерабочий легаси-шаблон, что выглядело как "фронт не деплоится"/битый киоск, поскольку киоск просто открывает тот же URL). Поэтому после правок React (или на голом хосте без `--build`) нужен именно `docker compose ... up --build`, простой `restart` фронтенд не обновит. Если `backend/app.py` всё же не находит `frontend/dist/index.html` (например, запуск `python app.py` вне Docker без `npm run build`) — пишет `[WARN]` в лог и отдаёт диагностическую страницу вместо дашборда, а не молча ломается.
