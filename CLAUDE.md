...
- **Первый запуск через Docker без собранного фронтенда:** `docker-compose.override.yml` применяется автоматически и бинд-маунтит `./frontend/dist` поверх собранного в образе React-приложения — если на хосте `frontend/dist` не собран (`npm run build` не запускался), монтируется пустая папка и React-сборка из образа "исчезает". Бэкенд это детектит (`backend/app.py`) и вместо дашборда отдаёт на `/` диагностическую страницу-заглушку с инструкцией плюс пишет `[WARN]` в лог контейнера — раньше молча отдавался нерабочий легаси-шаблон, что выглядело как "фронт не деплоится"/битый киоск (киоск просто открывает тот же URL). Чинится `cd frontend && npm install && npm run build` до/после `docker compose ... up`.
