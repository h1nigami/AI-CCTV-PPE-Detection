...
`DETECT_MODES = {people, ppe, faces}` (`backend/config.py`), персистятся в `data/detect_modes.json`, меняются через `PUT /api/detect-modes` / UI. Если все три выключены — YOLO пропускается целиком, но **стрим не останавливается**: `process_frame` отдаёт сырой кадр без оверлеев (ранний `return` возвращает полный кортеж из 7 элементов — иначе воркер падал на распаковке и переставал писать в `annotated_buffers` → `/video_frame` отдавал 204 → фронт «NO SIGNAL»). Выключение `people` каскадно гасит `ppe` и `faces`. Переключение `faces` стартует/глушит face-воркеры на лету. **Режим «только люди»** (ppe+faces выкл.): люди рисуются нейтральной рамкой без статусов/пропуска — детекция жеста не запускается (иначе при пустом списке требуемых СИЗ жест «ОК» выдавал «пропуск» всем).
