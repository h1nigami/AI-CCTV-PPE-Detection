...
- **Ключ траектории** (`mkey` в `process_frame`): `g{global_id}` если `global_id>0` (переживает переинициализацию трека и узнавание «со спины»), иначе `t{track_id}` (режим «только люди», лица выключены). Один стабильный ключ на человека в пределах камеры.
