...
# ── Отслеживание перемещений (кто сидит на месте / кто встал и куда пошёл) ──
# Геометрический трекинг по «точке ног» поверх готовых детекций людей: рисует
# траекторию движения и считает непрерывное время на месте. Пороги ниже
# масштабируются ростом человека (доля от высоты bbox). Реализация —
# backend/tracking/movement.py; правятся из UI через DETECTION_SETTINGS_SPEC.
MOVEMENT_STILL_FRAC = 0.15   # допуск «на месте» (доля роста)
MOVEMENT_MOVE_FRAC = 0.6     # смещение от якоря места, считающееся «встал»
MOVEMENT_SETTLE_SEC = 2.0    # сколько держаться смирно, чтобы зафиксировать место
MOVEMENT_TRAIL_SEC = 15.0    # длина следа траектории по времени (сек)
MOVEMENT_MAP_TRAIL_SEC = 60.0  # длина кросс-камерного следа на карте (сек)
...
DETECT_MODES: dict[str, bool] = {
    ...
    "ppe": True,
    "faces": True,
    # Отслеживание перемещений (траектория + время на месте). Зависит от people.
    "movement": True,
}
...
DETECTION_SETTINGS_SPEC = [
    ...
    {"key": "movement_still_frac", "default": MOVEMENT_STILL_FRAC, "type": "float",
     "min": 0.05, "max": 0.5, "step": 0.01,
     "group": "Отслеживание перемещений",
     "label": "Порог «на месте» (доля роста)", "unit": "",
     "desc": "Насколько человек может смещаться, всё ещё считаясь сидящим на месте "
             "(доля от его роста в кадре). Больше → терпимее к ёрзанью; меньше → "
             "строже, любое движение сбрасывает «сидит»."},
    {"key": "movement_move_frac", "default": MOVEMENT_MOVE_FRAC, "type": "float",
     "min": 0.2, "max": 2.0, "step": 0.05,
     "group": "Отслеживание перемещений",
     "label": "Порог «встал с места» (доля роста)", "unit": "",
     "desc": "На сколько нужно отойти от своего места (доля роста), чтобы засчитать, "
             "что человек встал и пошёл. Больше → фиксирует только явный уход; "
             "меньше → срабатывает от небольшого шага."},
    {"key": "movement_settle_sec", "default": MOVEMENT_SETTLE_SEC, "type": "float",
     "min": 0.5, "max": 15.0, "step": 0.5,
     "group": "Отслеживание перемещений",
     "label": "Время фиксации места", "unit": "сек",
     "desc": "Сколько секунд человек должен держаться смирно, чтобы точка считалась "
             "его местом (и начался отсчёт времени на месте)."},
    {"key": "movement_trail_sec", "default": MOVEMENT_TRAIL_SEC, "type": "float",
     "min": 2.0, "max": 120.0, "step": 1.0,
     "group": "Отслеживание перемещений",
     "label": "Длина следа траектории", "unit": "сек",
     "desc": "Сколько секунд движения показывать линией траектории на видео."},
    {"key": "movement_map_trail_sec", "default": MOVEMENT_MAP_TRAIL_SEC, "type": "float",
     "min": 5.0, "max": 600.0, "step": 5.0,
     "group": "Отслеживание перемещений",
     "label": "Длина следа на карте (кросс-камерный)", "unit": "сек",
     "desc": "Сколько секунд пути показывать на общей карте помещения — единая "
             "линия перемещения человека через все камеры (стич по личности)."},
]
