...
- **Архив (NVR)** — страница «Архив»: выбор камеры/даты, 24-часовой таймлайн сегментов записи (пометка движения) с метками событий; клик по событию → переход к моменту в записи; плеер с перемоткой
