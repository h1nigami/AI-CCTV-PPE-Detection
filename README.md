...
- **Голосовые предупреждения** — браузер озвучивает вход без СИЗ в опасную зону (Web Speech API, ru-RU, синтез на стороне клиента). ⚠️ Требует установленного в системе **русского TTS-голоса** — иначе озвучка пропускается (см. ниже)
...
> 🔊 **Голосовые предупреждения требуют русского TTS-голоса.** Озвучка идёт через
> Web Speech API **в браузере оператора** — синтез делает ОС, не бэкенд. Если в
> системе нет русского голоса, браузер пытается прочитать кириллицу английским
> голосом → выходит «быстрая неразборчивая речь», поэтому такая озвучка
> намеренно пропускается (в консоли — предупреждение `[voice] Русский голос
> синтеза не найден`). Установить голос:
...
> Голоса в браузере подгружаются асинхронно — фронт ждёт события `voiceschanged`
> и кэширует список, поэтому первое предупреждение тоже озвучивается корректно.
