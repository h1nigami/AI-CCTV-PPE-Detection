...
- **Re-ID лиц** — кросс-камерная идентификация через InsightFace (buffalo_l) с русскими именами
...
REID_SIM_THRESHOLD = 0.55        # порог косинусного сходства ArcFace
...
- **Re-ID (лица)** — распознавание запускается каждый `REID_FRAME_SKIP` (по умолч. 3) кадр для снижения нагрузки на GPU
