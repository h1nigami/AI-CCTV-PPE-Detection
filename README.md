...
- **Автообнаружение камер** — кнопка «Найти камеры»: поиск непаролёных RTSP в локальной сети (ONVIF WS-Discovery + скан подсети). **Работает без интернета** — сканируются подсети всех локальных интерфейсов (определяются через интерфейсы хоста, не зависят от маршрута в WAN). Опц. авто-добавление при старте (`CAMERA_AUTODISCOVER`)
