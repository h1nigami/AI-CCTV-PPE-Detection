...
# Singleton-локи от предыдущего запуска: контейнер мог упасть/перезапуститься,
# а профиль лежит в persistent-volume и переживает рестарт контейнера.
# Оставшийся SingletonLock заставляет Chromium думать, что профиль уже занят
# другим процессом, и он блокирует его от "повреждения". В контейнере
# гарантированно работает только один Chromium одновременно — снятие лока безопасно.
rm -f "$PROFILE_DIR"/Singleton*

# Системная шина D-Bus: в минимальном образе её никто не поднимает — без неё
# Chromium пишет "Failed to connect to the bus: ... system_bus_socket" в лог.
# Поднимаем свою, локальную для контейнера, чтобы убрать причину целиком.
