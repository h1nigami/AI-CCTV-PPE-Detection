...
# (см. предыдущие фиксы — CRLF в шебанге, отсутствие D-Bus), а профиль лежит
# в persistent-volume и переживает рестарт контейнера. Оставшийся
# SingletonLock заставляет Chromium думать, что профиль уже занят другим
# процессом, и он блокирует его от "повреждения". В контейнере гарантированно
# работает только один Chromium одновременно — снятие лока безопасно.
